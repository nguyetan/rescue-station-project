...
test('valid request to get LSCP', async (t) => {
  const param = requestSetup;

  const result = await api.post('/findLSCP', { data: param });
  if (result.ok) {
...
test('test valid type LSCP', async (t) => {
  const param = requestSetup;
...
test('test valid number poit result LSCP', async (t) => {
  const param = requestSetup;
  const result = await api.post('/findLSCP', { data: param });
  if (result.ok) {
    const data = result.data as any;
...
test('invalid request to get P-LSCP', async (t) => {
  const param = requestSetup;

  const result = await api.post('/findLSCP', { param });
  if (result.ok) {
    t.fail();
  } else {
    t.pass();
